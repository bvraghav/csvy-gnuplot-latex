...
| `overleaf` | collect `gnuplot-lua-tikz.sty`, `gnuplot-lua-tikz-common.tex` (generated by the local gnuplot with `set terminal tikz createstyle`), `gnuplotfit.sty` and every `NAME.gp.tex` into `overleaf/`, for a LaTeX project without gnuplot |
