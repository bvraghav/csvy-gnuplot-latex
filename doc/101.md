...
Every figure has a *natural size*, set in its `.csvy` as `fig: {w, h}` (in
cm). For the tested settings that's 8 cm × 4 cm. `\input` places the figure
...
- **Only stretch:** set `fig: {w, h}` in the `.csvy` to about the smallest
  size you need.
